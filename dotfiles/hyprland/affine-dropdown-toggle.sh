...
# Dropdown Affine toggle script for Hyprland
...
get_dropdown_window() {
    # Look for an Affine window (floating or not)
    hyprctl clients -j | jq -r ".[] | select(.class == \"$DROPDOWN_CLASS\") | .address" | head -n1
}
...
if [ -n "$DROPDOWN_WINDOW" ]; then
    ...
    if is_window_hidden "$DROPDOWN_WINDOW"; then
        ...
        hyprctl dispatch movetoworkspacesilent special:affine-dropdown,address:$DROPDOWN_WINDOW
    else
